...
def filter_colors(img_ori):
    # lower_red_color = np.array([0, 0, 110])
    lower_red_color = np.array([0, 0, 105])
    upper_red_colr = np.array([100, 100, 255])
    red_image = cv2.inRange(img_ori, lower_red_color, upper_red_colr)
    return red_image
...
def main():
    ...
    while cv_image != None:
      ...
      image_red = filter_colors(image_roi)
      ...
      if cameraCheck:
          if x_location != None:
              # if isFindFarObject and changing_line == 0:
                  # force_Stop()
                  ...
                  # tempCnt = tempCnt + 1
                  # accumulateObjectCnt = accumulateObjectCnt + 1
