...
*/
			...
			term = 10;
			if(find_yellow == 1 && time >= 0+term && time <25+term){
				differ = 270;
			}
			else if(find_yellow == 1 && time >= 25+term && time < 49+term){
				differ = -200;
			}
			else if(find_yellow == 1 && time == 49+term){
				
			}
			else if(find_yellow == 1 && time ==50+term){
				cout << "back back " << endl;
				differ = 999;
			}

			else if(find_yellow == 1 && time >= 51+term && time < 70+term){
				cout << "back back " << endl;
				
				differ = 999;
				
			}
			if(time >= 70+term && time < 100+term){
				cout << "time over" << endl;
				//cam_msg2.data = 2;
				//pub3.publish(cam_msg2);
				//loop_rate.sleep();
			...
			if(find_yellow == 1 && time >= 100+term && time < 145+term){
				cout << "go right "<<endl;
				angle_send = 20;
				cmd.angular.z = angle_send;
				pub2.publish(cmd);
			}
			else if(find_yellow == 1 && time >= 145+term && time < 165+term){
				cout << "go left " << endl;
				angle_send = 135;
				cmd.angular.z = angle_send;
				pub2.publish(cmd);
