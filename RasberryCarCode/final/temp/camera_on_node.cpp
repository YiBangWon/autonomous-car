...
void filter_colors_yellow(Mat img_original, Mat& img_filtered)
{
	...
	Scalar lower_white_color = Scalar(105, 215, 215);
...
int orange_detect(Mat &img){
	...
	Rect yellow_roi(0, 100, Width/2, 110);
...
//adaptiveThreshold(thresh,thresh,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY_INV,31,5);
	...
	for(int i = 0; i < roi_yellow.rows; i++){
		for(int j = 0; j < roi_yellow.cols; j++){
			if (image_yellow.at<Vec3b>(i, j)[0] >= 105 && image_yellow.at<Vec3b>(i, j)[0] <= 160) {
					if (image_yellow.at<Vec3b>(i, j)[1] >= 215 && image_yellow.at<Vec3b>(i, j)[1] <= 255) {
						if (image_yellow.at<Vec3b>(i, j)[2] >= 215 && image_yellow.at<Vec3b>(i, j)[2] <= 255) {
...
*/
			...
			if(find_yellow == 1 && time >= 0 && time <22){
				differ = 260;
			}
			else if(find_yellow == 1 && time >= 22 && time < 34){
				differ = -200;
			}
			...
			if(find_yellow == 1 && time >= 51 && time < 57){
				differ = -200;
			}
			else if(find_yellow == 1 && time >= 57 && time < 62){
				differ = 200;
			}
