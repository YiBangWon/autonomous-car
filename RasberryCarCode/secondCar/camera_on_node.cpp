...
int orange_detect(Mat &img){
	...
	//roi = img(rect_roi);
	Rect rect_roi(img.cols*0.1, img.rows*0.75, img.cols*0.85, img.rows*0.2);
	Mat image_roi = img(rect_roi);
	Rect yellow_roi(0, 100, Width/2, 110);
	Mat roi_yellow = img(yellow_roi);
	
	image_roi.convertTo(image_roi,-1,1,30);
...
//adaptiveThreshold(thresh,thresh,255,ADAPTIVE_THRESH_MEAN_C,THRESH_BINARY_INV,31,5);
	...
	for(int i = 0; i < roi_yellow.rows; i++){
		for(int j = 0; j < roi_yellow.cols; j++){
			if(roi_yellow.at<Vec3b>(i,j)[0] < 70 && roi_yellow.at<Vec3b>(i,j)[1] > 180 && roi_yellow.at<Vec3b>(i,j)[2] > 180){
				yellow_point++;
			}
		}
	}
	...
	*/
	  ...
	  for(;;){
		...
		//writer.write(frame);
			...
			if ((key = waitKey(60)) >= 0) {
				if (key == 27){
					cout<< "OOOOOOOOOOOHHHHHHHHHHHH" << endl;
					break;
				}
				else if (key = ' ') {
					capture = !capture;
